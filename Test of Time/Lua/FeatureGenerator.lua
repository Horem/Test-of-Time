...
function FeatureGenerator.Create(args)
	...
	-- Set feature traits.
	local iJunglePercent = args.iJunglePercent or 14; -- 12
		...
		for x = 0, self.iGridW - 1, 1 do
			...
			if(plot ~= nil) then
				...
				elseif(plot:IsWater() == true) then
					...
					elseif(TerrainBuilder.CanHaveFeature(plot, g_FEATURE_OASIS) == true and math.ceil(self.iOasisCount * 200 / self.iNumLandPlots) <= self.iOasisMaxPercent ) then
...
function FeatureGenerator:AddIceAtPlot(plot, iX, iY)
	...
	if Map.IsWrapX() and (iY == 0 or iY == self.iGridH - 1) then
		...
		-- 705: Reduce polar ice slightly, especially on coastal plots
		...
		if(terrainType == g_TERRAIN_TYPE_COAST) then		
			...
			if(rand < 8 * (lat - 0.9)) then -- 0.875
				TerrainBuilder.SetFeatureType(plot, g_FEATURE_ICE);
			elseif(rand < 3 * (lat - 0.75)) then -- 4 *
				TerrainBuilder.SetFeatureType(plot, g_FEATURE_ICE);
			end
		end
	end
...
function FeatureGenerator:AddMarshAtPlot(plot, iX, iY)
	...
	if(TerrainBuilder.CanHaveFeature(plot, g_FEATURE_MARSH)) then
		if(math.ceil(self.iMarshCount * 100 / self.iNumLandPlots) <= self.iMarshMaxPercent) then
			...
			-- 705: Modified base score from 300 and added check for rivers and coast. If a tile
			-- is on either river or coast it will have the same chance as defualt, lower if not
			-- on either and higher if a riverside coastal plot.
