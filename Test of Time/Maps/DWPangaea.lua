...
--	PURPOSE: Produces single massive continent.
------------------------------------------------------------------------------
...
function GeneratePlotTypes()
	...
	while done == false do
		-- 705: So... the random grain dice normally sucks.
		grain_dice = 2;
		rift_dice = 2;
		...
		local max = 0;
		local min = 0; 
		max = g_iH - math.ceil(g_iH * 0.25);
		min = math.ceil(g_iH * 0.25);
		...
		-- Now test the biggest landmass to make sure it isn't too small
...
function InitFractal(args)
	...
	-- 705: Reduce max size and hope that the plot shift function will clean up this mess.
	local g_maxH = math.floor(g_iH * 0.8);
